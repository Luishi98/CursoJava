...
SELECT nombre_cliente AS Cliente FROM cliente c INNER JOIN pedido p ON c.codigo_cliente = p.codigo_cliente WHERE p.fecha_entrega > p.fecha_esperada;
-- 10. Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.
SELECT p.gama AS Gama, c.nombre_cliente AS Cliente
FROM producto p 
INNER JOIN detalle_pedido dp ON dp.codigo_producto = p.codigo_producto 
INNER JOIN pedido ped ON ped.codigo_pedido = dp.codigo_pedido
INNER JOIN cliente c ON c.codigo_cliente = ped.codigo_cliente
